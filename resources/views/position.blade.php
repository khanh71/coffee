...
@if(Session::has('error')) <script>
    swal({
        title: "Lỗi",
        text: "Chức danh này đã được sử dụng, không thể xóa",
        icon: "error"
    });
</script> @endif
                ...
                <div class="row">
                    <form action="{{route('position')}}" method="post" class="col-md-9">
                        ...
                        <div class="form-group input-group">
                            <input name="search" type="text" class="form-control text-capitalize" placeholder="Nhập chức danh bạn cần tìm vào đây nhé..." value="{{$search}}" autofocus>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-uppercase" id="ModalLabel">Thêm chức danh</h5>
            ...
            <div class="modal-body">
                <form class="forms-sample" id="newPositionForm" method="post" action="{{route('new-position')}}">
                    ...
                    <div class="form-group">
                        <label for="posname">Tên chức danh</label>
                        <input type="text" class="form-control text-capitalize" id="posname" name="posname" required maxlength="50" value="{{old('posname')}}">
                    </div>
                    <div class="form-group">
                        <label>Hệ số lương</label>
                        <input type="number" class="form-control text-capitalize" id="coefficient" name="coefficient" required min='0' max='20' value="{{old('coefficient')}}">
                    </div>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-uppercase" id="ModalLabel">Sửa chức danh</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="forms-sample" id="editPositionForm" method="post" action="{{route('edit-position')}}">
                    ...
                    @endif
                    <div class="form-group">
                        <label for="posnameedit">Tên chức danh</label>
