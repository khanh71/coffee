@extends('app')
@section('title','Điều chỉnh phiếu nhập kho')
@section('css')
...
<script type="text/javascript">
    $('#datepicker-popup-daywork').datepicker('update', new Date('{{$imp->impdate}}'));
...
<div class="card">
    <div class="card-body">
        <div class="card-title ribbon ribbon-info"><div class="glow"></div>Điều chỉnh phiếu nhập</div>
        <div class="row">
            <form class="col-12" id="editImportForm" method="post" action="{{route('edit-import',$imp->idimp)}}">
                @if(count($errors->postEditImport_Error)>0)
                @foreach($errors->postEditImport_Error->all() as $err)
                ...
                @endif
                {{csrf_field()}}
                <input type="hidden" name="idimp" value="{{$imp->idimp}}">
                <div class="mb-3">
                    <fieldset>
                        <legend>thông tin phiếu nhập</legend>
                        <div class="form-group">
                            <label class="mr-2"><b>Nhà cung cấp:</b></label>
                            <select class="form-control form-control-md text-capitalize mr-5" id="idsupp" name="idsupp">
                                @foreach($supps as $supp)
                                <option value="{{$supp->idsupp}}" @if($supp->idsupp == $imp->suppid) selected @endif>{{$supp->suppname}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="mr-2"><b>Ngày nhập:</b></label>
                            <div id="datepicker-popup-daywork" class="input-group date datepicker">
                                ...
                                <input type="text" class="form-control" id="impdate" name="impdate" required>
                            </div>
                        </div>
                    </fieldset>
                </div>
                <div class="table-responsive mb-3">
                    <fieldset>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach($impdes as $impde)
                                <tr>
                                    <td>
                                        <select class="form-control form-control-md text-capitalize" id="idma" name="idma[]" required>
                                            <option value="" disabled="true">--chọn--</option>
                                            @foreach($mas as $ma)
                                            <option value="{{$ma->idma}}" @if($ma->idma == $impde->maid) selected @endif>{{$ma->maname}}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="number" class="form-control form-control-sm imptotal bg-white" id="imptotal" name="imptotal[]" readonly value="{{$impde->imptotal}}">
                                    </td>
                                    <td class="text-center">
                                        <input type="hidden" name="idimpde[]" value="{{$impde->idimpde}}">
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    ...
                                    <td><input type="text" class="form-control form-control-sm bg-white" id="total" name="total" readonly value="{{number_format($imp->total).'₫'}}"></td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
